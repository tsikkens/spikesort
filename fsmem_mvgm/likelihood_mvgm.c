/* likelihood_mvgm
  ...
  mex -output likelihood_mvgm.dll likelihood_mvgm.c
  
  Myself, I use Intel CPP compiler as : 

  mex  -f mexopts_intel10amd.bat -output likelihood_mvgm.dll likelihood_mvgm.c
...
void likelihood_mvgm(double * , double * , double * , double * , double * , int  , int  , int  , int  , int , 
				 ...
				 int nrhs, const mxArray *prhs[] )
{
	...
	dimsL         = (int *)mxMalloc(numdimsL*sizeof(int));
	...
	temp_sigma         = (double *)mxMalloc(m2*sizeof(double));
	
	temp_invsigma      = (double *)mxMalloc(m2*sizeof(double));
	
	det_sigma          = (double *)mxMalloc(d*sizeof(double));
	
	invsigma           = (double *)mxMalloc((m2*d)*sizeof(double));
 ...
 void likelihood_mvgm(double *Z , double *M , double *S , double *P , double *L , int d , int m , int K , int V , int R , 
